...
class Recorder:
    ...
    def _hud(self, img: np.ndarray, t: float, z: float, speed: float) -> None:
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.rectangle(img, (0, 0), (self.w, 46), (0, 0, 0), -1)
        cv2.putText(img, self.label, (14, 31), font, 0.72, (255, 255, 255), 2, cv2.LINE_AA)
        vert = f"altura {z:5.1f} m" if self.kind == "air" else f"profundidade {-z:4.1f} m"
        txt = (f"t {t:5.1f} s   " if t is not None else "") + f"{vert}   vel {speed:4.2f} m/s"
        (tw, _), _ = cv2.getTextSize(txt, font, 0.72, 2)
        cv2.putText(img, txt, (self.w - tw - 14, 31), font, 0.72, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.rectangle(img, (14, 56), (44, 70), self.trail_color, -1)
    ...
    def frame(self, env, state: dict, sim_t: float) -> None:
        ...
        if self._last_pos is not None and np.linalg.norm(loc - self._last_pos) > 0.02:
            ...
            env.draw_line(self._last_pos.tolist(), loc.tolist(), color=rgb, thickness=3.0, lifetime=0)
